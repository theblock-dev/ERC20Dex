...
module.exports = async (deployer, _network, accounts) => {
    ...
    const transferSeedToken = async (token,trader) => {
    ...
    await Promise.all(
      [dai,bat,rep,zrx].map(
        token => transferSeedToken(token,trader1))
    );

    await Promise.all(
        [dai,bat,rep,zrx].map(
          token => transferSeedToken(token,trader2))  
      );

    await Promise.all(
    [dai,bat,rep,zrx].map(
        token => transferSeedToken(token,trader3))  
    );
    await Promise.all(
    [dai,bat,rep,zrx].map(
        token => transferSeedToken(token,trader4))  
    );
